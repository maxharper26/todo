...
const TABS = ['Signals', 'Top Long', 'Top Short'];
...
export default function CotPanel() {
  ...
  const topLong   = data?.top_long  ?? [];
  const topShort  = data?.top_short ?? [];
  ...
  function TopTable({ rows, ratioLabel, colour }) {
    if (!rows.length) return <p style={{ color: '#3a3a52', fontSize: '0.85rem' }}>No data.</p>;
    return (
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
          <thead>
            <tr style={{ color: '#64748b', borderBottom: '1px solid #1e1e2e', textAlign: 'left' }}>
              ...
              <th style={{ padding: '6px 10px', textAlign: 'right' }}>{ratioLabel}</th>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.market} style={{ borderBottom: '1px solid #16161f', background: i % 2 === 0 ? '#111118' : '#16161f' }}>
                <td style={{ padding: '6px 10px', color: '#3a3a52', fontSize: '0.75rem' }}>{i + 1}</td>
                <td style={{ padding: '6px 10px', fontWeight: 700 }}>{r.market}</td>
                <td style={{ padding: '6px 10px', color: '#3a3a52', fontSize: '0.78rem' }}>{r.category}</td>
                <td style={{ padding: '6px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: colour, fontWeight: 700 }}>{r.ratio_now.toFixed(3)}</td>
                <td style={{ padding: '6px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: '#94a3b8' }}>{r.long.toLocaleString()}</td>
                <td style={{ padding: '6px 10px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: '#94a3b8' }}>{r.short.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div style={{ marginBottom: 24, background: '#111118', border: '1px solid #1e1e2e', borderRadius: 8, color: '#e2e8f0' }}>
      ...
      {open && (
        <div style={{ padding: '0 18px 18px' }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 14 }}>
            {TABS.map(t => (
              <button
                ...
                onClick={() => setTab(t)}
                style={{
                  fontSize: '0.75rem', fontWeight: 600, padding: '3px 12px', borderRadius: 4, cursor: 'pointer',
          ...
          {!loading && tab === 'Signals' && (
            sorted.length === 0
              ? <p style={{ color: '#3a3a52', fontSize: '0.85rem' }}>No signals.</p>
              : <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                    <thead>
                      <tr style={{ color: '#64748b', borderBottom: '1px solid #1e1e2e', textAlign: 'left' }}>
          ...
          {!loading && tab === 'Top Long'  && <TopTable rows={topLong}  ratioLabel="Long Ratio"  colour="#22c55e" />}
          {!loading && tab === 'Top Short' && <TopTable rows={topShort} ratioLabel="Short Ratio" colour="#ef4444" />}
        </div>
      )}
    </div>
  );
